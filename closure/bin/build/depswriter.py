...
def _GetPair(s):
  ...
  try:
    ...
    raise Exception(f'Unable to parse input line as a pair: {s}')


def main():
  """CLI frontend to MakeDepsFile."""
  logging.basicConfig(format=f'{sys.argv[0]}: %(message)s', level=logging.INFO)
  ...
  out = open(options.output_file, 'w') if options.output_file else sys.stdout
